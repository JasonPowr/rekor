...
var (
	...
	enableLastIndexTracking = flag.Bool("enable-last-index-tracking", false, "Enable tracking of the last filled index in Redis, if enabled start index does not need to be set")
)

const (
	lastFilledIndexKey = "last_filled_index"
)
...
func main() {
	...
	if *enableLastIndexTracking && *startIndex != -1 {
		log.Fatal("enableLastIndexTracking and startIndex cannot be set simultaneously")
	}
	if *startIndex == -1 && !*enableLastIndexTracking {
	...
	redisClient := newRedisClient(0) //default db
	lastFilledRedisClient := newRedisClient(1)
	...
	var lastFilled int
	if *enableLastIndexTracking && !*dryRun {
		lastFilled, err = getLastFilledIndex(ctx, lastFilledRedisClient)
		if err != nil {
			log.Fatalf("Failed to retrieve last filled index: %v", err)
		}
		if lastFilled == -1 {
			log.Printf("%s not found creating entry", lastFilledIndexKey)
			*startIndex = 0
		} else {
	...
	if *enableLastIndexTracking && !*dryRun {
		if err := setLastFilledIndex(ctx, lastFilledRedisClient, *endIndex); err != nil {
...
func getLastFilledIndex(ctx context.Context, redisClient *redis.Client) (int, error) {
	val, err := redisClient.Get(ctx, lastFilledIndexKey).Result()
	if err != nil {
		if err == redis.Nil {
...
func setLastFilledIndex(ctx context.Context, redisClient *redis.Client, index int) error {
	...
	err := redisClient.Set(ctx, lastFilledIndexKey, indexStr, 0).Err()
	if err != nil {
